  ...
  <main>
    ...
    <section id="harga-section" class="harga-section fade-element" style="padding-top: 5rem;">
      ...
      <div class="card-list">
        ...
        @foreach($layanan_by_kategori as $kategori => $items)
          <div class="card fade-element">
            <h3>{{ ucwords(strtolower($kategori)) }}</h3>
            <ul>
              @foreach($items as $item)
                <li class="layanan-item">
                  <span class="nama">{{ $item->nama_layanan }}</span>
                  <span class="harga">
                    @if($item->is_flexible == 1)
                      Rp {{ number_format($item->harga_min, 0, ',', '.') }} –
                      Rp {{ number_format($item->harga_max, 0, ',', '.') }} / {{ $item->satuan }}
                    @else
                      Rp {{ number_format($item->harga_satuan, 0, ',', '.') }} / {{ $item->satuan }}
                    @endif
                  </span>
                </li>
              @endforeach
        ...
        {{-- BAGIAN DISKON TETAP HARDCODE SESUAI PERMINTAAN ANDA --}}
        <div class="card fade-element">
